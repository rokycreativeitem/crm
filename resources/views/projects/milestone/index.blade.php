@push('title', get_phrase('Milestone'))
    ...
    <div class="ol-card-body">
        ...
        <div class="row mt-2 mb-2 print-d-none">
            <div class="col-md-6 d-flex align-items-center gap-3 d-none" id="table-action-btns">
                ...
                <div>
                    <a href="#" onclick="rightCanvas('{{ route(get_current_user_role() . '.milestone.create', ['code' => request()->route()->parameter('code')]) }}', 'Create milestone')" class="btn ol-btn-outline-secondary d-flex align-items-center cg-10px">
                            ...
                            <td>
                                <div class="dAdmin_profile d-flex align-items-center min-w-200px">
                                    <div class="dAdmin_profile_name">
                                        <h4 class="title fs-12px">{{ $milestone->title }}</h4>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <div class="dAdmin_profile d-flex align-items-center min-w-200px">
                                    <div class="dAdmin_profile_name">
                                        <h4 class="title fs-12px">{!! $milestone->description !!}</h4>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <div class="dAdmin_profile d-block align-items-center min-w-200px">
                                    <span class="p-2 fs-12px">{{ get_task_progress($milestone->id) }}%</span>
                                    <div class="progress ms-2">
                                        <div class="progress-bar bg-primary" role="progressbar" style="width: {{ get_task_progress($milestone->id) }}%; " aria-valuenow="{{ get_task_progress($milestone->id) }}" aria-valuemin="0" aria-valuemax="100"></div>
                                    </div>
                                </div>
                            </td>
                            <td class="cursor-pointer" onclick="rightCanvas('{{ route(get_current_user_role() . '.milestone.tasks', ['id' => $milestone->id]) }}', 'Milestone')">
                                <div class="dAdmin_profile d-flex align-items-center min-w-200px">
                                    <div class="dAdmin_profile_name">
                                        <h4 class="title fs-12px tasks-box">{{ count($milestone->tasks) }}
                                            {{ get_phrase('Tasks') }}</h4>
                                    </div>
                                </div>
                            </td>
                            <td class="print-d-none">
                                <div class="dropdown ol-icon-dropdown ol-icon-dropdown-transparent">
                                    ...
                                    <ul class="dropdown-menu">
                                        <li>
                                            <a class="dropdown-item" onclick="rightCanvas('{{ route(get_current_user_role() . '.milestone.edit', $milestone->id) }}', 'Edit milestone')" href="#">{{ 'Edit' }}</a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" onclick="confirmModal('{{ route(get_current_user_role() . '.milestone.delete', $milestone->id) }}')" href="javascript:void(0)">{{ get_phrase('Delete') }}</a>
                                        </li>
                                    </ul>
                                </div>
                            </td>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '#delete-selected', function() {
                ...
                if (selectedIds.length > 0) {
                    multiDelete("{{ route(get_current_user_role() . '.milestone.multi-delete') }}", selectedIds);
