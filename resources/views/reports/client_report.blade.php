                                        ...
                                        <tr data-id="{{ $payment->id }}" class="context-menu">
                                            ...
                                            <td>
                                                <div class="dAdmin_profile d-flex align-items-center min-w-200px">
                                                    <div class="dAdmin_profile_name">
                                                        @php
                                                            $client_id = App\Models\Project::distinct('client_id')->pluck('client_id')->toArray();
                                                        @endphp
                                                        @foreach ($client_id as $id)
                                                            <h4 class="title fs-14px" data-client-name="{{ get_user($id)->name }}">{{ get_user($id)->name }}</h4>
                                                        @endforeach
                                                    </div>
                                                </div>
                                            </td>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.context-menu').forEach(row => {
                let clientName = row.querySelector('[data-client-name]')?.getAttribute('data-client-name');
                let payment = row.querySelector('[data-client-payment]')?.getAttribute('data-client-payment');

                if (clientName && payment) {
                    dataArr.push({
                        x: clientName,
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.context-menu').forEach(row => {
                ...
                if (clientName && payment) {
                    ...
                    payments.push(parseFloat(payment)); // Convert payment to a number
                }
            });

            // Check if there's data to display
            if (clientNames.length && payments.length) {
                const ctx = document.getElementById('pieChart').getContext('2d');

                const pieChart = new Chart(ctx, {
                    type: 'pie',
                    data: {
                        labels: clientNames, // Dynamic labels from collected data
                        datasets: [{
                            label: clientNames,
                            data: payments, // Dynamic data for chart
                            backgroundColor: ['#ff5733', '#33ff57', '#3357ff', '#ff33a6', '#33d1ff'], // Add more colors if needed
                            borderColor: '#ffffff',
                            borderWidth: 1,
                        }]
                    },
