                ...
                <tr>
                    ...
                    <td>
                        @foreach (json_decode($notification_row->template, true) as $user_type => $template)
                            <p>{{ get_phrase('To ' . $user_type) }}: {!! $template !!}
                            </p>
                        @endforeach
                    </td>
                    <td>
                        <a onclick="showRightModal('{{ route('modal', ['admin.setting.edit_email_template', 'id' => $notification_row->id]) }}', '{{ get_phrase('New user registration') }}')" class="btn ol-btn-primary btn-rounded" href="javascript:void(0)" data-toggle="tooltip" data-placement="top"
                            title="{{ get_phrase('Edit email template') }}" style="min-width: 40px;">
