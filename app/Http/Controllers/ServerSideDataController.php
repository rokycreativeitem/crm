...
class ServerSideDataController extends Controller
{
    ...
    {
        $query = Payment_history::select('project_code', DB::raw('MAX(date_added) as date_added'), DB::raw('SUM(amount) as total_amount'))->groupBy('project_code');
        ...
        return datatables()
            ...
            ->addColumn('date', function ($report) {
                return date('d m y', $report->date_added);
            })
            ->addColumn('project', function ($report) {
                $project = Project::where('code', $report->project_code)->first();
                return $project->title;
            })
