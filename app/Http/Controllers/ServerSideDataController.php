...
class ServerSideDataController extends Controller
{
    ...
    public function milestone_server_side($string) {
    ...
    public function task_server_side($string) {
        $query = Task::query();
        if (!empty($string)) {
            $query->where(function ($q) use ($string) {
                $q->where('title', 'like', "%{$string}%");
    ...
    public function file_server_side($string) {
        $query = File::query();
        if (!empty($string)) {
            $query->where(function ($q) use ($string) {
                $q->where('title', 'like', "%{$string}%");
        ...
        return datatables()
            ...
            ->addColumn('date', function ($file) {
                return date('d-M-y', strtotime($file->timestamp_start));
            })
            ->addColumn('updated_by', function ($file) {
                return User::where('id', $file->user_id)->first()->name;
            })
            ->addColumn('downloaded', function ($file) {                
                return '<a href="" class="download-btn"><svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
